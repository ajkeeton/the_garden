...
def main():
    ...
    threads = []
    for i in range(5):
        print(f"Connecting to server at {server_address}:{server_port} ({i+1})")
        t = threading.Thread(target=do_client, args=(server_address, server_port, mac + f"_{i}"), daemon=True)
        t.start()
        threads.append(t)
